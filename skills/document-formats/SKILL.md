...
description: 문서를 읽거나 만들거나 다른 형식으로 바꿀 때 사용한다. 한글(.hwp)·구형 오피스(.doc/.ppt/.xls)·PDF를 읽는 법, 긴 PDF에서 볼 쪽만 골라내 값을 아끼는 법, 결과를 PDF로 내보내는 법, 그리고 엑셀(.xlsx)을 만들 때의 글자 크기 규칙, 발표자료(.pptx)를 만들 때 항상 지켜야 하는 글꼴·윤곽선 규칙과 이미 잘못 만들어진 파일을 고치는 법을 담았다. **.pptx 를 만들 때는 증상이 보이지 않아도 반드시 연다** — 한국어 슬라이드는 기본값이 틀려 있어 사용자가 알아차린 뒤에는 늦다. 문서를 읽으려다 실패했을 때, PDF를 만들 때도 확인한다.
---
...
반드시 적는다' 절을 따른다. 기본값은 PC마다 다르니 그냥 두면 읽는 쪽에서 멈출 수 있다.
